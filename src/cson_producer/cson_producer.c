...
t_flush_buffer_func flush_func, void *dest_ptr)
{
	...
	return (cson_produce_node(&producer, cson, 0));
}
...
const unsigned char *src, size_t srcsize)
{
	...
	while (srcsize > 0)
	{
		...
		ft_memcpy(producer->buffer, src, nbytes);
		producer->buffer_position += nbytes;
		src += nbytes;
		srcsize -= nbytes;
		if (producer->buffer_position >= CSON_PRODUCER_BSIZE)
		{
			flush_func(producer->buffer, producer->buffer_position, producer->dest_ptr);
			producer->buffer_position = 0;
		}
	}
...
static size_t			cson_produce_indentation(t_cson_producer *producer, unsigned int indent_depth)
{
	int		i;
	size_t	bytes_produced;
...
const t_cson *cson, unsigned int indent_depth)
{
	...
	bytes_produced = cson_produce_indentation(producer);
	if (cson->key != NULL)
	{
		bytes_produced += cson_produce(producer, "\"", 1);
		bytes_produced += cson_produce(producer, cson->key, ft_strlen(cson->key));
		bytes_produced += cson_produce(producer, "\": ", 3);
	}
	...
	bytes_produced += cson_produce(producer, "\n", 1);
	return (bytes_produced);
}
